...
from .window.window_factory import WindowFactory
from .performer.logs_performer import LogsPerformer
from .performer.data_performer import DataPerformer


class Application():
    def __init__(self, app_dir):
        self.__app_dir = app_dir
        self.__lockfile_dir = f'{self.__app_dir}/temp/app.lock'
        self.__logfile_dir = f'{self.__app_dir}/config/log.log'
        
        self.logs_perf = LogsPerformer(self.__logfile_dir)
        self.win_factory = WindowFactory(self.logs_perf)
        self.data_perf = None
        
    def acquire_lock(self):
        lockfile_dir = os.path.dirname(self.__lockfile_dir)
        os.makedirs(lockfile_dir, exist_ok=True)
        
        self.fd = open(self.__lockfile_dir, 'w')
        try:
    ...
    def try_to_update(self):
        ...
        self.data_perf = DataPerformer(self.__app_dir, main_window, self.logs_perf)
        self.data_perf.load_service_data()
        
        latest_version = self.data_perf.service_data[self.data_perf.latest_version_key]
        archive_filename = f'Network.Folders.v.{latest_version}.Setup.zip'
        archive_file_dir = f'{self.__app_dir}/{archive_filename}'
        
        if os.path.exists(archive_file_dir):
            self.logs_perf.log(self.logs_perf.INFO, self.logs_perf.FOUND_UPDATER_MESS_ID, (archive_filename,))
    ...
    def start(self):
        ...
        Thread(
            ...
            args=(self.data_perf, self.__app_dir),
